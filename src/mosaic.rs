...
use crate::{QuadRgba, NilRgba, TileSet, NearestTile};


pub fn render_4to1(source_img: &RgbaImage, tile_set: &TileSet<QuadRgba>, tile_size: u32, tint_opacity: f64) -> RgbaImage {
...
pub fn render_random(source_img: &RgbaImage, tile_set: &TileSet<NilRgba>, tile_size: u32, tint_opacity: f64) -> RgbaImage {
    let mut output = RgbaImage::new(source_img.width() * tile_size, source_img.height() * tile_size);
    ...
    for tile_y in 0..source_img.height() {
        for tile_x in 0..source_img.width() {
            ...
            match resize_cache.get(path) {
                Some(tile_img) => {
                    imageops::overlay(&mut output, tile_img, tile_x * tile_size, tile_y * tile_size);
                },
                _ => {
                    let tile_img = image::open(path).unwrap();
                    let tile_img = imageops::resize(&tile_img, tile_size, tile_size, FilterType::Lanczos3);
            ...
            fill_rect(&mut output, &pixel, &(tile_x * tile_size, tile_y * tile_size, tile_size, tile_size));
        }
    }
    output
}
